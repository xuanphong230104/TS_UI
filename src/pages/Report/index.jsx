...
const EODReport = (props) => {
  ...
  const fetchDailyCheckIn = async (team) => {
    try {
      ...
      // Generate initial report content based on tasks
      generateReportContent(tasksWithStatus);
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (hasReported) {
        await axiosClient.put(API_ENDPOINTS.REPORT_BY_ID.replace("{id}", todayReport.id), {
          team_id: teamId,
          check_in_id: dailyReport.id,
          content: reportContent,
          tasks: checkinTasks.map((task) => ({
            id: task.id,
            title: task.title,
            description: task.description,
            status: task.status,
            progress: task.completionStatus,
            priority: task.priority,
          })),
        });
      } else {
        await axiosClient.post(API_ENDPOINTS.REPORT, {
          ...
          tasks: checkinTasks.map((task) => ({
            ...
            description: task.description,
            status: task.status,
            progress: task.completionStatus, // Use completionStatus as progress
            priority: task.priority,
          })),
        });
      }
                      ...
                      <Input
                        ...
                        onChange={(e) => {
                          ...
                          if (!isNaN(value) && value >= 0 && value <= 100) {
                            ...
                            // Immediately generate updated report content
